...
//     document.querySelector('button').outerHTML = '<button id="play_btn" onclick="firstPage()" >Restart!</button>';
...
function desert_init(){
    ctx.drawImage(backImg,0,0,1500,700);
    ctx.drawImage(backImg,-1500,0,1500,700);
    ctx.fillText("SCORE : "+score,5,20);
}
function pilot_init(){
    ctx.drawImage(pilotImg2,100,350,100,100);
}
function enemy_init(){
    enemyList = [];
    for(var i=0; i<enemyList.length;i++){
...
document.addEventListener("keyup", keyUpHandler, false);
    ...
    if(rightPressed) {
        ...
        if(stopMove == 1){
            clearInterval(movePlane);}
    } var mv =setInterval(movePlane,10) 
